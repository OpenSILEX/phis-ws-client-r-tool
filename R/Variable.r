  ...
  public = list(
    ...
    fromJSON = function(VariableJson) {
      ...
      if (!is.null(VariableObject$`trait`)) {
        ...
        traitObject$fromJSON(jsonlite::toJSON(VariableObject$trait, auto_unbox = TRUE))
        self$`trait` <- traitObject
      }
      if (!is.null(VariableObject$`method`)) {
        methodObject <- Method$new()
        methodObject$fromJSON(jsonlite::toJSON(VariableObject$method, auto_unbox = TRUE))
        self$`method` <- methodObject
      }
      if (!is.null(VariableObject$`unit`)) {
        unitObject <- Unit$new()
        unitObject$fromJSON(jsonlite::toJSON(VariableObject$unit, auto_unbox = TRUE))
