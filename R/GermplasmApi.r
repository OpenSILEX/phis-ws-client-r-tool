...
#' @description opensilexWSClient.Germplasm
#'
...
#' \describe{
#'
#' get_germplasm_by_search Get all germplasm corresponding to the search params given
#'
#'
#' post3 Post Germplasm,
#'
#' }
#'
#' @export
GermplasmApi <- R6::R6Class(
  ...
  public = list(
    ...
    get_germplasm_by_search = function(page_size,page,germplasm_uri,germplasm_label,germplasm_type,from_genus,from_species,from_variety,from_accession,language,...){
      args <- list(...)
      queryParams <- list()
      ...
      if (!missing(`germplasm_uri`)) {
        queryParams['germplasmURI'] <- germplasm_uri
      }

      if (!missing(`germplasm_label`)) {
        queryParams['germplasmLabel'] <- germplasm_label
      }

      if (!missing(`germplasm_type`)) {
        queryParams['germplasmType'] <- germplasm_type
      }

      if (!missing(`from_genus`)) {
        queryParams['fromGenus'] <- from_genus
      }

      if (!missing(`from_species`)) {
        queryParams['fromSpecies'] <- from_species
      }

      if (!missing(`from_variety`)) {
        queryParams['fromVariety'] <- from_variety
      }

      if (!missing(`from_accession`)) {
        queryParams['fromAccession'] <- from_accession
      }
